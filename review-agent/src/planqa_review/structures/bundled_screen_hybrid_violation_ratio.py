...
from planqa_review.structures.fewshot_bank import EXCEPTION_EXAMPLES, VIOLATION_EXAMPLES_RATIO
from planqa_review.tiers import ABSENCE_CHECK_RULE_IDS
from planqa_review.verifier import is_reference_excused_by_rule

# bundled_screen_hybrid_violation_ratio — bundled_screen_hybrid과 완전히 동일(2단계, 문단형,
# 콜통합, 룰텍스트+퓨샷)하지만 위반 예시만 VIOLATION_EXAMPLES(2개) 대신
# VIOLATION_EXAMPLES_RATIO(3개)로 준다. bundled_screen_hybrid_ratio(예외 예시만 1->2)의
# 대칭 축 — 룰 텍스트는 "무엇이 위반인지"를 이미 담고 있어 위반 예시는 보조 신호에 가깝지만,
# 예외조건은 한 줄 텍스트(exception_text)뿐이라 예외 예시의 정보 기여가 더 클 수 있다는
# 가설과, 그 반대로 위반 예시를 늘리는 쪽이 더 도움이 될 수도 있다는 가설을 각각 독립
# 셀로 갈라서 비교하기 위한 것.
...
def _hybrid_block(rule: RuleDef) -> str:
    ...
    for example in VIOLATION_EXAMPLES_RATIO.get(rule.rule_id, []):
...
def review_document(
    ...
    """bundled_screen_hybrid_violation_ratio — bundled_screen_hybrid과 완전히 동일(2단계,
    문단형, 콜통합, 룰텍스트+퓨샷)하지만 위반 예시만 2개 대신 3개를 준다."""
