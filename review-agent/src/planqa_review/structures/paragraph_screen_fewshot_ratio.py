...
# paragraph_screen_fewshot_ratio — 퓨샷 "위반/예외 비율 조정" 축 전용 셀. paragraph_screen_
# fewshot(재선정된 기준선: 위반 2개+예외 1개)과 완전히 동일하되, 딱 하나만 다르다 — 예외
# 예시를 EXCEPTION_EXAMPLES(1개)가 아니라 EXCEPTION_EXAMPLES_RATIO(2개)로 준다. 위반
# 예시·판정 방식·청킹은 전부 동일하게 유지해서, 두 구조를 같은 문서로 파일럿하면 정확도
# 차이가 "예외 예시 개수" 하나만의 순수 효과로 해석될 수 있다(2026-08-10 재설계).
...
def review_document(
    ...
    """paragraph_screen_fewshot_ratio — paragraph_screen_fewshot과 완전히 동일(2단계
    screen→confirm, 문단형, 위반 예시 2개)하지만 예외 예시만 1개 대신 2개를 준다. 이 구조
    하나만 바꿔서 "위반/예외 비율"의 순수 효과를 측정하기 위한 셀. `max_workers`를 안 주면
    (기본값) 그 패스의 카테고리 수만큼 한꺼번에 병렬 실행, `max_workers=1`을 주면 순차
    실행(테스트 결정성용)."""
