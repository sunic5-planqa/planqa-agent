...
# paragraph_screen_hybrid — paragraph_screen(룰텍스트만)과 paragraph_screen_fewshot(퓨샷만)
# 사이의 세 번째 콘텐츠 조합: 둘을 안 고르고 **둘 다** 준다(룰텍스트 + 퓨샷 예시). 퓨샷만
# 줬을 때 헛지적은 줄지만 진짜 위반까지 놓치는 현상이 확인됐는데, 룰텍스트(공식 정의)가
# 여전히 있으면 그 놓침이 없어지는지, 있으면서 퓨샷 예시가 경계 사례 판단에 도움이 되는지를
# 확인하기 위한 셀. 정적/동적 퓨샷 방향을 정하기 전에 먼저 검증한다.
...
_SCREEN_HYBRID_SYSTEM = (
    ...
    "against the numbered rules and list every span that might violate one, however "
    "uncertain.\n"
...
_CONFIRM_HYBRID_SYSTEM = (
    ...
    "this time, whether the flagged span actually violates the rule — the screening pass "
    ...
    "above are really the same underlying problem, confirm violated=true on only ONE of "
...
def _screen_category(chunks: list[Chunk], rules: list[RuleDef], global_context: str, llm: LLMClient) -> list[_Candidate]:
    ...
    prompt = f"Rules to check (text + examples):\n{rule_block}\n\nReturn the candidates JSON."
...
def review_document(
    ...
    """paragraph_screen_hybrid — paragraph_screen/paragraph_screen_fewshot과 판정 방식·
    청킹은 동일(2단계screen→confirm, 문단형)하지만, screen/confirm 양쪽 프롬프트에 룰
    텍스트와 fewshot 예시를 함께 준다. `max_workers`를 안 주면(기본값) 그 패스의 카테고리
