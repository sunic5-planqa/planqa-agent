...
# direct_verdict — ① 판정 단계 수 실험의 통제 변형. cell3(2단계: screen→confirm, 콜분리×
# 룰전부, 위계형)와 세분화·청킹을 완전히 동일하게 유지한 채 판정 단계 수만 1단계로 줄인다
# — 카테고리마다 독립된 콜(콜분리)은 그대로 두고, screen+confirm 2콜을 한 콜로 합쳐서 룰
# 전문을 처음부터 다 주고 바로 최종 판정을 받는다. proposal0(직접판정형, 콜통합)과는
# 세분화 축이 다르므로, cell3와만 순수하게 "단계 수" 하나만 다른 비교쌍이 된다.

# Duplicated rather than imported from models/gemini_lite/context.py, same as cell3.py —
# that module is baseline structure code (제안5) this structure must not depend on or
# touch, per the additive-only rule.
...
def _review_category(
    ...
    goes through a private `isolate_client` copy instead — see instrumentation.py's
    `record_call` docstring for why the shared instance can't be used directly under
    concurrency. The copy's usage is folded back onto the original below."""
