...
def _review_category(
    ...
    candidates = record_call(
        ...
        rule_ids=rule_ids,
        events=events,
        call=lambda: _screen_category(chunks, rules, global_context, screen_llm),
    )
    if not candidates:
        return []
    ...
    return record_call(
        ...
        events=events,
        call=lambda: _confirm_category(candidates, chunks, rules_by_id, doc_id, level, global_context, source_text, rulebook, confirm_llm),
    )
