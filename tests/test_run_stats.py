...
def test_build_run_stats_aggregates_both_clients():
    ...
    stats = build_run_stats(
        profile="gemini_lite", backend="gemini", screen_llm=screen_llm, confirm_llm=confirm_llm, total_wall_seconds=10.0
    )
...
def test_build_run_stats_handles_no_calls_yet():
    screen_llm = _FakeLLM("screen-model", [])
    confirm_llm = _FakeLLM("confirm-model", [])

    stats = build_run_stats(
        profile="gemini_lite", backend="gemini", screen_llm=screen_llm, confirm_llm=confirm_llm, total_wall_seconds=0.0
    )
