> **`expr/` 폴더 안내 (2026-08-12)**: 이 폴더는 `feature/review-agent` 브랜치(pforu의 구조/
> 콘텐츠 축 어블레이션 실험 전체)를 그대로 옮겨온 **독립 실험 공간**이다. 백엔드에
> 벤더링·배포되는 `services/review-agent/`(canonical, main 승격 완료분)와는 완전히 분리되어
> 있고, 루트 `pyproject.toml`의 uv workspace 멤버(`services/*`/`tools/*`/`packages/*`)에도
> 안 잡혀서 자체 `pyproject.toml`/`uv.lock`으로 따로 논다 — 이 안에서 뭘 바꿔도 배포된
> 코드나 워크스페이스 전체 설치엔 영향이 없다. 여기서 나온 실험이 실제로 채택되면 그때
> `services/review-agent/`로 별도 PR을 통해 반영한다(자동 동기화 없음).
