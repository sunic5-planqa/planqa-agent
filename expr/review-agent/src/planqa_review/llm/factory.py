...
def build_llm_client(
    ...
    per memory: planqa-model-selection-policy). "anthropic" calls Claude directly (own paid
    credit); "gateway" calls it through the shared mindlogic gateway instead. `temperature`
    defaults to 0.0 (not each backend's own default) for ablation-run reproducibility."""
