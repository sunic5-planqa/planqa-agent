...
from planqa_review.structures.fewshot_bank import EXCEPTION_EXAMPLES
...
# paragraph_screen_dynamic_fewshot — paragraph_screen_fewshot(정적 퓨샷: 룰마다 항상 같은
# 큐레이션된 위반 예시 2개)과 판정 방식·청킹은 동일하지만, 위반 예시를 그 패스의 chunk_block
# 전체와 가장 비슷한 것으로 그때그때 검색해서 고른다(fewshot_retrieval.top_k_examples,
# 문자 2-gram 유사도) — 검색 대상은 fewshot_bank.ALL_VIOLATION_CANDIDATES(uncapped 전체
# 풀). 예외 예시는 이번엔 정적(EXCEPTION_EXAMPLES, 1개) 그대로 유지 — 위반 예시만 동적으로
# 바뀐다(2026-08-10, 퓨샷 상세 구성 3번째 축).
...
_SCREEN_FEWSHOT_SYSTEM = (
    ...
    "check it against the rules (learn each rule_id's pattern from its examples) and list "
    "every span that might violate one, however uncertain.\n"
...
_CONFIRM_FEWSHOT_SYSTEM = (
    ...
    "precisely this time, whether the flagged span actually matches the violation pattern "
    "shown for that rule_id — the screening pass over-flags on purpose, so most candidates "
    ...
    "contradiction. If several of the candidates above are really the same underlying "
...
def _fewshot_block(rule: RuleDef, reference_text: str) -> str:
    lines = [f"  {rule.rule_id} ({rule.category_label}):"]
    for example in top_k_examples(reference_text, rule.rule_id, k=_TOP_K_VIOLATIONS):
...
def _screen_category(chunks: list[Chunk], rules: list[RuleDef], global_context: str, llm: LLMClient) -> list[_Candidate]:
    ...
    # The dynamic examples are picked per-rule against this pass's whole chunk_block (not
    # per individual chunk) — a coarser v1 granularity, see module docstring.
    fewshot_block = "\n".join(_fewshot_block(rule, chunk_block) for rule in rules)
    context_block = f"Document context:\n{global_context}\n\n" if global_context else ""
    # context_block+chunk_block is identical across every category dispatched in this pass
    # (only `rules` — and therefore which examples get retrieved — differs), same sharing
    # opportunity as the static fewshot structures use for Anthropic prompt caching.
    cache_prefix = f"{context_block}Chunks:\n{chunk_block}"
    prompt = f"Rules (id + examples, no rule text):\n{fewshot_block}\n\nReturn the candidates JSON."

    response = llm.complete_json(system=_SCREEN_FEWSHOT_SYSTEM, prompt=prompt, cache_prefix=cache_prefix)
...
def review_document(
    ...
    """paragraph_screen_dynamic_fewshot — paragraph_screen_fewshot과 판정 방식·청킹은
    동일(2단계screen→confirm, 문단형)하지만, 위반 예시를 정적 캡이 아니라 그 패스의 chunk_
    block과 가장 비슷한 예시로 그때그때 검색해서 고른다(fewshot_retrieval.top_k_examples).
    예외 예시는 정적(EXCEPTION_EXAMPLES) 그대로. `max_workers`를 안 주면(기본값) 그 패스의
    카테고리 수만큼 한꺼번에 병렬 실행, `max_workers=1`을 주면 순차 실행(테스트 결정성용)."""
