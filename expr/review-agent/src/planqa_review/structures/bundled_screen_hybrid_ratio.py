...
# bundled_screen_hybrid_ratio — bundled_screen_hybrid과 완전히 동일(2단계, 문단형, 콜통합,
# 룰텍스트+퓨샷)하지만 예외 예시만 EXCEPTION_EXAMPLES(1개) 대신 EXCEPTION_EXAMPLES_RATIO
# (2개)로 준다 — hybrid가 이긴 콘텐츠 축 위에서 비율조정의 순수 효과를 보기 위한 셀.
...
def _hybrid_block(rule: RuleDef) -> str:
    ...
    for exception_example in EXCEPTION_EXAMPLES_RATIO.get(rule.rule_id, []):
        lines.append(
...
def review_document(
    ...
    """bundled_screen_hybrid_ratio — bundled_screen_hybrid과 완전히 동일(2단계, 문단형,
    콜통합, 룰텍스트+퓨샷)하지만 예외 예시만 1개 대신 2개를 준다."""
