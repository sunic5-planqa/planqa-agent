...
# bundled_screen_dynamic_fewshot — bundled_screen_fewshot(정적 퓨샷)과 판정 방식·청킹은
# 동일(2단계, 문단형, 카테고리 안 나눔)하지만, 위반 예시를 그 패스의 chunk_block 전체와
# 가장 비슷한 것으로 그때그때 검색해서 고른다(fewshot_retrieval.top_k_examples). 예외
# 예시는 정적(EXCEPTION_EXAMPLES) 그대로 유지 — 위반 예시만 동적으로 바뀐다.
...
def review_document(
    ...
    """bundled_screen_dynamic_fewshot — bundled_screen_fewshot과 판정 방식·청킹은 동일
    (2단계, 문단형)하지만, 위반 예시를 정적 캡이 아니라 그 패스의 chunk_block과 가장 비슷한
    예시로 그때그때 검색해서 고른다. 예외 예시는 정적 그대로."""
