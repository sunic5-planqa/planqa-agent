...
# bundled_screen — ①(재검증 후 2단계로 재확정)과 ③(콜통합)을 합친 셀. bundled_verdict(콜통합
# ×룰전부×1단계)와 청킹은 동일(문단형)하게 유지하되, 판정을 1콜로 곧장 끝내지 않고 cell3/
# paragraph_screen과 같은 screen→confirm 2단계로 나눈다 — 다만 카테고리별로 쪼개지 않고
# 한 패스(문단/문서)에 배정된 모든 룰을 screen 1콜, confirm 1콜에 그대로 묶는다.
...
_SCREEN_SYSTEM = (
    ...
    "(which may span several review categories at once) and list every span that might "
    "violate one, however uncertain.\n"
...
_CONFIRM_SYSTEM = (
    ...
    "Decide, precisely this time, whether each flagged span actually violates its rule — "
    ...
    "words or at different levels of detail is NOT a conflict; only flag a genuine logical "
...
def _screen_pass(chunks: list[Chunk], rules: list[RuleDef], global_context: str, llm: LLMClient) -> list[_Candidate]:
    rule_block = "\n".join(f"{rule.rule_id} ({rule.category_label}): {rule.text}" for rule in rules)
...
def review_document(
    ...
    """bundled_screen — bundled_verdict와 청킹은 동일(문단형)하지만, 카테고리별로 쪼개지
    않고 한 패스(문단/문서)에 배정된 모든 룰을 screen 1콜 + confirm 1콜의 2단계로 판정한다."""
