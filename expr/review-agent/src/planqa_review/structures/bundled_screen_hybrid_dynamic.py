...
from planqa_review.structures.fewshot_bank import ALL_EXCEPTION_CANDIDATES
...
# bundled_screen_hybrid_dynamic — bundled_screen_hybrid과 판정 방식·청킹·콘텐츠(룰텍스트+
# 퓨샷)는 동일하지만, 위반 예시와 예외 예시 둘 다 정적 캡(VIOLATION_EXAMPLES/
# EXCEPTION_EXAMPLES) 대신 그 패스의 chunk_block 전체와 가장 비슷한 것으로 그때그때
# 검색해서 고른다(fewshot_retrieval.top_k_examples, 각각 uncapped ALL_VIOLATION_CANDIDATES/
# ALL_EXCEPTION_CANDIDATES 풀에서 검색). 개수는 baseline과 동일(위반 2개/예외 1개) — 바뀌는
# 건 선정 방식뿐이라 "전체에서 비슷한 예시를 가져오는" 효과만 순수하게 격리한다.
...
_CONFIRM_HYBRID_SYSTEM = (
    ...
    "every other category, or if no specific second location can be identified. When "
...
def _hybrid_block(rule: RuleDef, reference_text: str) -> str:
    ...
    for exception_example in top_k_examples(reference_text, rule.rule_id, k=_TOP_K_EXCEPTIONS, candidates=ALL_EXCEPTION_CANDIDATES):
        lines.append(
...
def _screen_pass(chunks: list[Chunk], rules: list[RuleDef], global_context: str, llm: LLMClient) -> list[_Candidate]:
    ...
    rule_block = "\n".join(_hybrid_block(rule, chunk_block) for rule in rules)
...
def review_document(
    ...
    """bundled_screen_hybrid_dynamic — bundled_screen_hybrid과 판정 방식·청킹·콘텐츠
    (룰텍스트+퓨샷)는 동일하지만, 위반 예시와 예외 예시 둘 다 정적 캡이 아니라 그 패스의
    chunk_block과 가장 비슷한 예시로 그때그때 검색해서 고른다."""
