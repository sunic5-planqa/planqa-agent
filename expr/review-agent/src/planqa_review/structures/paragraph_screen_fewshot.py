...
# paragraph_screen_fewshot — paragraph_screen(새 기준 구조: 2단계screen→confirm + 문단형
# 청킹)과 판정 방식·청킹은 동일하게 유지하고, 룰 텍스트를 screen/confirm 양쪽 모두에서
# 완전히 빼고 fewshot_bank.py의 예시만 준다 — category_fewshot과 동일한 취지지만 스크리닝
# 단계가 추가된 버전.
...
def review_document(
    ...
    """paragraph_screen_fewshot — paragraph_screen과 판정 방식·청킹은 동일(2단계screen→
    confirm, 문단형)하지만 screen/confirm 양쪽 프롬프트 모두에서 룰 텍스트를 빼고
    fewshot_bank.py의 예시만 준다. `max_workers`를 안 주면(기본값) 그 패스의 카테고리
    수만큼 한꺼번에 병렬 실행, `max_workers=1`을 주면 순차 실행(테스트 결정성용)."""
