...
from planqa_review.structures import bundled_screen_hybrid_dynamic, fewshot_retrieval
from planqa_review.structures.bundled_screen_hybrid_dynamic import review_document
from planqa_review.structures.fewshot_bank import FewShotException, FewShotExample
...
def test_review_document_picks_the_excused_example_most_similar_to_the_document(rulebook_path, monkeypatch):
    # k=1 in this structure, so a pool of exactly 2 lets us tell "picked" from "excluded".
    monkeypatch.setattr(
        bundled_screen_hybrid_dynamic,
        "ALL_EXCEPTION_CANDIDATES",
        {
            "MI-01": [
                FewShotException("zzzqqq999@@@ 완전히 무관함", "무관 조건", "전혀 무관한 근거"),
                FewShotException("결제 버튼을 누르면 즉시 처리됩니다 예외 사례", "결제 예외 조건", "결제 버튼 관련 근거"),
            ]
        },
    )
    rulebook = parse_rulebook(rulebook_path)
