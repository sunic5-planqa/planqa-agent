...
from planqa_review.structures.paragraph_screen_fewshot import review_document
...
def test_review_document_never_leaks_rule_text_into_either_stage(rulebook_path):
    """The whole point of this structure: neither the screen nor the confirm prompt may
    contain the rule's own defined text/exception wording — only rule_id + curated
    fewshot examples."""
    rulebook = parse_rulebook(rulebook_path)
    screen_llm = _ContentAwareLLM("MI-01")
    confirm_llm = _ContentAwareLLM("MI-01")
    mi01_rule_text = rulebook.rules["MI-01"].text

    review_document("DOC-TEST", _DOC, rulebook, screen_llm, confirm_llm, max_workers=1)

    for call in screen_llm.calls + confirm_llm.calls:
        assert mi01_rule_text not in call["prompt"]
        assert mi01_rule_text not in call["cache_prefix"]
