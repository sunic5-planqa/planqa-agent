...
## 2026-08-12 — `verifier.py`의 §3 참조-예외 정규식이 프로즈 인용을 놓치는 버그 발견/수정

발표 슬라이드용 "예외조건 O건 중 O건 방어" 수치를 API 비용 없이 계산하려고, 결정론적으로
검증 가능한 4개 규칙(AE-01/GA-03/LG-03/TC-02, `rulebook.reference_exception_rule_ids`)의
"예외조건 data" 테스트 행(`qa_dataset_frozen.xlsx` Review4, 79~107행 중 4건)에 대해
`has_valid_reference_exception()`을 직접 돌려봄.

### Done

- **버그 발견**: `_CITATION` 정규식이 룰북 §3 자체 예시 문구("반품/교환 정책서(DOC-005)
  참고")만 본떠서 `문서명(DOC-XXX) 참고/참조` 형태만 인식 — 그런데 "예외조건 data"
  테스트 행 4건은 전부 프로즈 인용("「5G 가족결합 운영전략서」 2-4 '...'을 따른다",
  "...에 해당하는 경우") 스타일이라 DOC 코드도 없고 참고/참조로 끝나지도 않음. 수정 전
  4/4 전부 "예외 인식 안 됨"으로 오탐.
- `_CITATION`을 `_CITATION_DOC_CODE`(기존) + `_CITATION_NATURAL`(신규, 「제목」+섹션번호+
  따른다/해당하는 경우/참조 표기 꼬리)로 분리, `_has_citation()` 헬퍼로 OR 결합. eval-agent
  의 `verifier.py`(이 파일의 원본)에도 동일하게 반영 — 두 사본이 갈라지지 않게.
- eval-agent `test_verifier.py`에 신규 회귀 테스트 2개 추가(프로즈 인용 두 스타일 각각),
  기존 DOC-006 반례 테스트(정확한 인용이라도 지적 대상과 다른 문단이면 예외 아님) 그대로
  통과 확인 — 새 패턴이 기존 안전장치를 깨지 않음.
- review-agent 124/124 + eval-agent 86/86(신규 2건 포함) 전체 통과.
- 수정 후 4/4 전부 정확히 예외로 인식됨 — "예외조건 4건 중 4건 방어 성공"(단, 이 4건은
  §3 결정론적 체크가 있는 규칙 한정치이고, 나머지 25건은 LLM 판단 영역이라 이 수치에
  안 들어감).

### Next

- 나머지 25건(LG-01/02/04/05, LF-01~05, TC-01/04, TM-03, AE-03/04, MI-02×2/05~08,
  RD-01×2/03, GA-07)은 결정론적 체크가 없어 실제 리뷰 파이프라인 실행(API 비용) 없이는
